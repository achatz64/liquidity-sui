// async function cetus_pools_old(limit=30, offset=0) {
...
async function cetus_pools(limit=30, offset=30) {
  const payload = {
    filter: "verified",
    sortBy: "vol",
    sortOrder: "desc",
  ...
  const response = await (await fetch("https://api-sui.cetus.zone/v3/sui/clmm/stats_pools", {
    "headers": {
      "accept": "*/*"
    },
