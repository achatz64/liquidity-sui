// import { steam_liquidity } from "../../config/packages";
...
// export interface ConfigManagerSteam extends ConfigManagerWithClientAndLiquidityContract {
//     dex: Dex.Steam,
//     steam_api_wait_ms: number,
//     threshold_liquidity_usd_for_pool: number,
//     update_liquidity_ms: number,
//     pools_per_sui_liquidity_fetch_call: number
// }

// export class PoolManagerSteam extends PoolManagerWithClientAndLiquidityContract {
//     last_call_steam_api: number
//     config: ConfigManagerSteam
//     constructor(config: ConfigManagerSteam) {
//         if (config.dex != Dex.Steam) throw new Error(`${Dex.Steam} manager called with ${config.dex} dex argument`)
//         super(config);
//         this.config = config;
//         this.last_call_steam_api = 0;
//     }

//     async call_steam_pool_api(condition_for_pool: (pool_info: SteamBasicPoolInfo) => boolean): Promise<SteamBasicPoolInfo[]> {
//         let stop_requesting_pages: boolean = false;
//         const pools: SteamBasicPoolInfo[] = [];

//         // loop not used 
//         while (!stop_requesting_pages) {
//             if (Date.now()-this.last_call_steam_api > this.config.steam_api_wait_ms) {
//                 const response: SteamApiPoolsResponse  = await (await fetch("https://api.mmt.finance/pools/v3", {
//                     "headers": {
...
//                     "method": "GET"
//                     })).json();
//                 this.last_call_steam_api = Date.now();
//                 if (response.status == 200) {
//                     logger(this.config.debug, LogLevel.DEBUG, LogTopic.PROPOSE_POOLS, `Read Steam pools`)
...
//                 }
//                 else {
//                     logger(this.config.debug, LogLevel.ERROR, LogTopic.PROPOSE_POOLS, `Steam API call failed with code ${response.status}: ${response.msg}`)
//                 }
//             }
//         }      
//         return pools;
...
//     condition_for_pool(pool_info: SteamBasicPoolInfo): boolean {
//         return Number(pool_info.tvl) > this.config.threshold_liquidity_usd_for_pool
//     }

//     parse_basic_pool_info(pool_info: SteamBasicPoolInfo): Pool {
//         const pool: Pool = {
...
//         const response = await this.call_steam_pool_api((pool_info) => this.condition_for_pool(pool_info));
...
//         const package_id = steam_liquidity; 
...
//         try {
//             const steam_liquidity = await this.create_liquidity_fetch_txn_and_simulate(pools);
//             if (steam_liquidity.length != pools.length) {
//                 throw new Error("Fetched liquidity does not match pools")
//             }
//             steam_liquidity.forEach((ticks, i)=> {pools[i].liquidity = ticks});
...
//   tokenX: SteamCoinInfo,
//   tokenY: SteamCoinInfo,
// }

// interface SteamApiPoolsResponse {
//     status: number,
//     msg: string,
//     data: SteamBasicPoolInfo[]   
// }
